...
#define APP_ADC_CHANNEL ADC_CHANNEL_5  //This is for GPIO 33 (see datasheet...)
...
void app_main(void)
{
    ...
    xTaskCreate(spi_stuff,
                "spi_stuff",
                STACK_SIZE, NULL,
                (UBaseType_t) 1,
                &xHandle);

    xTaskCreate(adc_read_value,
                "adc_read_value",
                STACK_SIZE, NULL,
                tskIDLE_PRIORITY,
                &xHandle);

    xTaskCreate(update_outputs,
                "update_outputs",
                STACK_SIZE, NULL,
                tskIDLE_PRIORITY,
                &xHandle);
...
esp_err_t set_adc(void){
    ...
    adc_oneshot_chan_cfg_t configChannel = {
        .atten = ADC_ATTEN_DB_11,
        .bitwidth = ADC_BITWIDTH_12
    };
    adc_oneshot_config_channel(adc1_handle, APP_ADC_CHANNEL, &configChannel);
    return ESP_OK;
}
...
void update_outputs(void *pvParameters){
    while (1){
        gpio_set_level(GPIO_LED_0, IOTables.digTbl[1][0] & 0b0000000000000001);
        gpio_set_level(GPIO_LED_1, IOTables.digTbl[1][0] & 0b0000000000000010);
        gpio_set_level(GPIO_LED_2, IOTables.digTbl[1][0] & 0b0000000000000100);

        gpio_set_level(GPIO_LED_3, IOTables.digTbl[1][1] & 0b0000000000000001);
        gpio_set_level(GPIO_LED_4, IOTables.digTbl[1][1] & 0b0000000000000010);
        gpio_set_level(GPIO_LED_5, IOTables.digTbl[1][1] & 0b0000000000000100);
        taskYIELD();
    }
}
